...
def generate_sample_data(zip_code, poi, date, num_samples=100):
    data = []
...
# Generate and save sample foot traffic data for each POI in each ZIP code for each day of June 2024
all_data = []
total_samples_per_day = 500  # Increase this number to generate more records

for day in range(1, 31):
    date = f"06/{day:02d}/2024"
    for zip_code in zip_codes:
        for poi in walmart_pois:
            foot_traffic_df = generate_sample_data(zip_code, poi, date, num_samples=total_samples_per_day)
            all_data.append(foot_traffic_df)

# Combine all data into one DataFrame and save it
combined_df = pd.concat(all_data, ignore_index=True)
filename = "Walmart_Foot_Traffic_June_2024_Large.csv"
